    ...
    return (
        <div className="flex items-start gap-4">
            ...
            <div className="w-full max-w-3xl space-y-4">
                {/* Summary Message */}
                <div className="p-4 rounded-lg shadow-md bg-white/80 dark:bg-gray-800/60 backdrop-blur-sm text-gray-800 dark:text-gray-200 border border-gray-300 dark:border-white/10">
                    <p className="leading-relaxed">{message.summary}</p>
                    <p className="text-xs mt-2 text-gray-600 dark:text-gray-400">Overall Similarity Score: {message.overallSimilarity}%</p>
                        ...
                        {message.responses.map((res, index) => {
                            ...
                            return (
                                <div key={index} className="grid grid-cols-6 gap-4 items-center">
                                    <div className={`col-span-2 font-medium ${isBaseline ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                ...
                <div className="p-4 rounded-lg shadow-md bg-white/80 dark:bg-gray-800/60 backdrop-blur-sm border border-gray-300 dark:border-white/10">
                    ...
                    {/* Tab Content */}
                    <div>
                        <h4 className="font-semibold text-gray-800 dark:text-white">{message.responses[activeTab].model}</h4>
                        ...
                        <p className="leading-relaxed text-gray-800 dark:text-gray-200">{message.responses[activeTab].response}</p>
                    </div>
                </div>
            </div>
        </div>
